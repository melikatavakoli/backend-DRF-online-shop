...
class TransactionAdminList(ListAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = Transaction.objects.select_related("user").annotate(
...
class CreatePendingTransaction(APIView):
    ...
    def post(self, request):
        ...
        if Transaction.objects.filter(
            user=request.user, status=TransactionStatus.pending
        ).exists():
            return Response(
        ...
        if total_today_transactions >= 500000 and not request.user.verified_national_id:
            return Response(
                {
                    "message": "To charge more than 500,000 Tomans, you must verify your national ID."
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        elif (
            total_today_transactions + int(amount) > 500000
            and not request.user.verified_national_id
        ):
            return Response(
                {
                    "message": "To charge more than 500,000 Tomans, you must verify your national ID."
            ...
            return Response(
                {
                    "message": "To charge more than 1,500,000 Tomans, you must verify your account."
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        return Response(
            {"transaction_id": transaction.id}, status=status.HTTP_201_CREATED
        )
...
class VerifyTransaction(APIView):
    ...
    def post(self, request):
        ...
        if response_json.get("status") == "success":
            ...
            transaction.track_id = track_id
            transaction.save(update_fields=["status", "track_id", "updated_at"])
...
class PaymentCallback(APIView):
    ...
    def get(self, request):
        ...
        return redirect(
            f"https://yourdomain.com/payment-result?trackId={track_id}&success={success}"
        )
